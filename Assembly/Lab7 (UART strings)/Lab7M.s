...
;R1: charaters to write
			;Initailize UART
			BL	 UART1__Init
			;Load base memory adress to store strings
			LDR	 R0, =String			
Main        ;Prompt user
			BL	 PutPrompt
			BL	 GetString
			;After string retirved, echo with proper format
			PUSH {R0}
			MOVS R0, #"\\"
			BL   PutChar
			POP  {R0}
...
GetString	PUSH {R0-R2,LR}
			MOVS R1, R0
			MOVS R2, #0
			;While string is not at max size, keep taking characters
NextCharS	CMP  R2, #MAX_STRING
			BEQ  Max_Len
			BL   GetChar
			BL   PutChar
			;If carraige return, terminate and return from call
			CMP  R0, #0x0D
			BEQ  CarReturn
			STRB R0,[R1,#0]
			ADDS R1, #0x01
			...
			;If at max lenght, do nothing untill carriage return
Max_Len		BL   GetChar
			CMP  R0, #0x0D
			BNE  Max_Len

			;Terminate and return from call 
...
NextCharP	LDRB R0, [R1, #0]
		    ;Move address to next character, check for null termination
			ADDS R1, #0x01
		    CMP  R0, #0x00
		    BEQ  Terminate
			BL   PutChar
			B	 NextCharP
			;When null found return to caller
...
Count		LDRB R0, [R1, #0]
			...
			ADDS R1, #0x01
		    CMP  R0, #0x00  
			;If null found return from call
			BEQ	 endString
			;Else add one to counter and continue
			ADDS R1, #1
			B	 Count
endString   POP {R0}
			BX   LR
...
;Strings that will be writen repeatively
...
;Declare a space for the string operations
String		SPACE	MAX_STRING+1
			ALIGN
;end variables here
__EndMyData          
            END