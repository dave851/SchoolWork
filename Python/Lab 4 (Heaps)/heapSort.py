...
def _first_of_3(heap, index):
    ...
    if rt < heap.size:        # If there are both left and right children
        ...
        or heap.compareFunc(rVal, thisVal):
            if not(heap.compareFunc(lVal, rVal)):
...
def heapify(heap):
    """
    Takes an invalid heap and in-place modifyes it to be a 
    valid heap
    
    heapify(heap) -> None
    """
    curr=parent(heap.size-1)
    while curr >= 0:
        Par = heap.array[curr]
        if rChild(curr) < heap.size and lChild(curr) < heap.size:
            LC = heap.array[lChild(curr)]
            RC = heap.array[rChild(curr)]
            if heap.compareFunc(RC, Par) or heap.compareFunc(LC, Par):
                if LC > RC:
                    (heap.array[lChild(curr)], heap.array[curr]) = (heap.array[curr], heap.array[lChild(curr)])
                    siftDown(heap, lChild(curr))
                else:
                    (heap.array[rChild(curr)], heap.array[curr]) = (heap.array[curr], heap.array[rChild(curr)])
                    siftDown(heap, rChild(curr))
        else:
            if rChild(curr) > heap.size-1:
                LC = heap.array[lChild(curr)]
                if heap.compareFunc(LC, Par):
                    (heap.array[lChild(curr)], heap.array[curr]) = (heap.array[curr], heap.array[lChild(curr)])
                    siftDown(heap, lChild(curr))
        if curr == 1:
            curr = 0
...
def heapFromList(lst):
    """
    Takes in a list and returns a valid max heap
    
    heapFromList(list) -> heap
    """
    heap = Heap(len(lst), greater)
    heap.array = deepcopy(lst)
    heap.size = len(lst)
    heapify(heap)
    return heap
...
def removeTop(heap):
    """
    Takes in a valid heap and returns the top element of the
    heap. Heap is updated to no long contain the element in a 
    valid form
    
    removeTop(heap) -> Element of heap
    """
    res = heap.array[0]
    heap.size = heap.size - 1
    heap.array[0] = heap.array[heap.size]
...
def sortedListFromHeap(heap):
    """
    Takes in a valid heap and returns a sorted list of elements
    
    sortedListFromHeap(heap) -> List
    """
    out=[]
    while heap.size > 0:
        out.append(removeTop(heap))
    return out

def heapSort(Lst):
    """
    Takes in a list and returns the list sorted, uses the heap
    structure to sort
    
    heapSort(List) -> List
    """
    heap=heapFromList(Lst)
    heapify(heap)
    return sortedListFromHeap(heap)
...
if __name__ == '__main__':
    ...
    data2=[1,2,3,4]
    data3=[2,3,1,4]
    print('Test heapFromList (Case 1) result: '+str(heapFromList(data1)))
    print('Test heapFromList (Case 2) result: '+str(heapFromList(data2)))
    print('Test heapFromList (Case 3) result: '+str(heapFromList(data3)))
    print('')
    h1=heapFromList(data1)
    h2=heapFromList(data2)
    h3=heapFromList(data3)
    h1=removeTop(h1)
    h2=removeTop(h2)
    h3=removeTop(h3)
    print('Test removeTop (Case 1) result: '+str(h1))
    print('Test removeTop (Case 2) result: '+str(h2))
    print('Test removeTop (Case 3) result: '+str(h3))
    print('')
    print('Test heapSort (Case 1) result: '+str(heapSort(data1)))
    print('Test heapSort (Case 2) result: '+str(heapSort(data2)))
    print('Test heapSort (Case 3) result: '+str(heapSort(data3)))
